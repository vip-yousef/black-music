...
from MatrixMusic import Zelzaly
...
— — — — — — — — — — — —
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton("‹ رجوع ›", callback_data="allgame2 " + str(m.from_user.id))],
        [InlineKeyboardButton("‹ 𝖱𝗌𝖤𝗑𝖲 𝖳𝖾𝖠𝗆 ›",
                              url=f"https://t.me/VVIZINN")],
    ])
...
async def games2(c: Client, m: CallbackQuery):
    ...
    await m.message.edit_text("≭︰أهلا بيك بقائمه العاب سورس احمد قم باختيار احدى الازرار\n↯", reply_markup=keyboard)
            ...
            if sudo2(m):
                async for photo in c.iter_profile_photos(m.from_user.id, limit=1):
                    await m.reply_photo(photo.file_id,
                                        caption="≭︰نسبه جمالك هي ~⪼ 500% مطور هذا عيب")
            else:
                gmalnumbers = [
                ...
                async for photo in c.iter_profile_photos(m.from_user.id, limit=1):
                    await m.reply_photo(photo.file_id,
                                        caption="≭︰نسبه جمالك هي ~⪼ " + random.choice(gmalnumbers) + "%")
